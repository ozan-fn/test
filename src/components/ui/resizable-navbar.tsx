...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
    ...
    return (
        <motion.div onMouseLeave={() => setHovered(null)} className={cn("absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2", className)}>
            {items.map((item, idx) => (
...
export const MobileNavToggle = ({ isOpen, onClick }: { isOpen: boolean; onClick: () => void }) => {
    return isOpen ? <XIcon className="text-black dark:text-white" onClick={onClick} /> : <MenuIcon className="text-black dark:text-white" onClick={onClick} />;
};
